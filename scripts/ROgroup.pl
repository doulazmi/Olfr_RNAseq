	...
	if ($InROflag2==1 ) {if ($clusteravant == 103899241) {$clusteravant=3567398;}
						print FIC_OP3 "$chr\t$clusteravant\t$clusterend\tClusterOlfrOrient$clusternb\t$k[4]\t$signevant\n"; 
	...
	if ($InROflag==1 &  !(exists $tab{$k[3]}) )  {
									...
									if ($k[3]=~/^Gm/ or $k[3]=~/^AL/ or $k[3]=~/^AC/ or $k[3]=~/^CT/ or $k[3]=~/^OR/ or $k[3]=~/Rik$/) {print FIC_OP4 "$k[3]\n";}
							 		...
							 		print FIC_OP2 "noRO-$k[3]\t $ligne2[$index]\t $tailleCDS\t $intervaleCDS\n"; 
							 		$InROflag= 0; $clusteravant=$cluster; $cluster=$gene_stop; $clusterend=$k[1]; $signevant=$signe;
									$InROflag2=1;
												}
	if (exists $tab{$k[3]})	{ 
							...
							if ($InROflag==0 ) { 
											if ($olfr=~/^Gm/ or $olfr=~/^AL/ or $olfr=~/^AC/ or $olfr=~/^CT/ or $olfr=~/^OR/ or $olfr=~/Rik$/) {print FIC_OP4 "$olfr\n";}
												print FIC_OP2 $ligne_avant; $chr=$k[0];$cluster= $gene_stop;  $signe =$k[5];
												}

							print FIC_OP2 "$tab{$k[3]}\t $ligne2[$index]\t$tailleCDS\t $intervaleCDS\n";
							$InROflag= 1;
						}

	if ( $InROflag==1 &  $signe ne $k[5]) { 
										if ($clusteravant ne $cluster) {
											 $clusteravant=$cluster; $cluster=$gene_stop; $clusterend=$k[1]; $signevant=$signe;
											$InROflag2=1;						   }
										}
 	
	$ligne_avant= "noRO-$k[3]\t $ligne2[$index]\t $tailleCDS\t $intervaleCDS\n";
	$gene_stop= $k[2];
	$gene_start=$k[1];
	$signe =$k[5];
	$chr=$k[0];
