...
rnaSeq() {
	...
	mkdir -p $OUT
	timeout 2h bash -c "NewRNAseqAnalysis/Software/STAR/bin/Linux_x86_64/STAR --runThreadN 7 --genomeDir NewRNAseqAnalysis/indexSTARmice --readFilesIn ${IN}/${1}_1.fastq.gz ${IN}/${1}_2.fastq.gz \
	...
	echo "NewRNAseqAnalysis/Software/STAR/bin/Linux_x86_64/STAR --runThreadN 7 --genomeDir NewRNAseqAnalysis/indexSTARmice --readFilesIn ${IN}/${1}_1.fastq.gz ${IN}/${1}_2.fastq.gz \
                                        --readFilesCommand zcat --outFileNamePrefix ${OUT}/job --outSAMtype BAM Unsorted --genomeLoad LoadAndKeep \
                                        --outSAMstrandField intronMotif --outFilterIntronMotifs RemoveNoncanonical --alignIntronMax 23000"
